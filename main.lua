...
It handles argument parsing and implements the program's commands using the modules `benmet.features` and `benmet.util`.
--]]
...
do
	...
	for k,v in pairs(arg) do
		min_index = min_index <= k and min_index or k
	end
...
local help_list_commands = function()
	print("usage: "..program_invocation_string_without_args.." <command> <further-args...>"
		...
		.."\n\navailable commands:")
	local ordered_commands_list = { -- ordered semantically
		'add-repo',
		'step.do',
	...
	local expected_command_count = #ordered_commands_list
	for i = 1, expected_command_count do
		local command = ordered_commands_list[i]
		if program_command_structures[command] then -- check consistency with actual program command list
			local summary = program_command_structures[command].summary
			print("   "..command..(summary and " : "..summary or " (no command summary available)"))
			program_command_structures[command] = nil
		else
